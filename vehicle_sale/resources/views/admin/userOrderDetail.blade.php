                        ...
                        <div class="wrap-table-shopping-cart">
                            <table class="table-shopping-cart">
                                <tr class="table_head">
                                    ...
                                    <th class="column-1" style="text-align: center">RESİM</th>
                                    ...
                                    <th class="column-5" style="text-align: center">İŞLEM</th>

                                </tr>
                                {{--                                $sorgu1=\App\Models\sepet::where("musteriNo",$items['kullaniciSiparisId'])->first();--}}
                                {{--                                $sorgu2=\App\Models\Products::where("id",$sorgu1['urunKodu'])->first();--}}
                                {{--                                $siparisDurum=\App\Models\OrderStatus::where("id",$items['siparisDurum'])->first();--}}
                                {{--                                $kullanici=\App\Models\Users::where("id",$items['kullaniciSiparisId'])->first();--}}

                                @foreach($sepet as $items)
                                    @php $sorgu1=""; @endphp
                                    @if($items['durum']=="1")
                                        @php
                                            $sorgu1=\App\Models\Products::where("id",$items['urunKodu'])->first();
                                            $kullanici=\App\Models\Users::where("id",$items['musteriNo'])->first();
                                            $siparis=\App\Models\Order::where("kullaniciSiparisId",$kullanici['id'])->first();
                                            $siparisDurum=\App\Models\OrderStatus::where("id",$siparis['siparisDurum'])->first();
                                ...
                                @if($sorgu1!="")
                                    <tr class="table_row " style="text-align: center;">
                                        <td class="column-0"
                                            style="text-align: center">{{$kullanici['isim']}} {{$kullanici['soyisim']}}</td>

                                        <td class="column-1">

                                            <img src="{{asset($sorgu1['resimUrl'])}}" alt="IMG" width="150px">

                                        </td>
                                        <td class="column-2">1</td>
                                        <td class="column-3">{{$sorgu1['fiyat']}} ₺</td>
                                        <td class="column-4" style="text-align: center">{{$sorgu1['aciklama']}}</td>
                                        <td class="column-5"

                                            style="text-align: center">{{$siparisDurum['aciklama']}}</td>

                                        <td class="column-6"

                                            style="text-align: center">
                                            <form action="{{route('orderstatu')}}" method="POST">
                                                @csrf
                                                <input name="siparisid" type="hidden" value="{{$siparis['kullaniciSiparisId']}}">
                                                <button type="submit" name="approve" class="flex-c-m stext-101 cl2 size-119 bg8 bor13 hov-btn3 p-lr-15 trans-04 pointer m-tb-10">Onayla</button>
                                                <button type="submit" name="denied" class="flex-c-m stext-101 cl2 size-119 bg8 bor13 hov-btn3 p-lr-15 trans-04 pointer m-tb-10">Reddet</button>
                                              </form>
